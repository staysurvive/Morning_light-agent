...
class KnowledgeBase(BaseModel):
    ...
    name: Mapped[str] = mapped_column(String(200), unique=True, index=True, comment="知识库名称")
    description: Mapped[str | None] = mapped_column(
        String(500), nullable=True, comment="描述"
    )
    status: Mapped[str] = mapped_column(
        String(50), default="empty", index=True, comment="状态: ready/indexing/error/empty"
    )
    ...
    embedding_model: Mapped[str] = mapped_column(
        String(100), default="text-embedding-ada-002", comment="Embedding 配置标识（预留）"
    )
    ...
    retrieval_strategy: Mapped[str] = mapped_column(
        String(20), default="fulltext", comment="检索策略: fulltext"
    )
...
class Document(BaseModel):
    ...
    file_name: Mapped[str] = mapped_column(String(500), index=True, comment="文件名")
...
class Segment(BaseModel):
    ...
    document: Mapped["Document"] = relationship(
        "Document", back_populates="segments", lazy="selectin"
    )
